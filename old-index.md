...
import {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (input.mr && this.isInAir && this.canDoubleJump && 
          ...
          if (this.featherJumpsLeft > 0) {
            ...
            // Notify player
            this.world.chatManager.sendPlayerMessage(player, 'Double Jump!', '00FFFF');
          } else {
            ...
            this.world.chatManager.sendPlayerMessage(player, 'No feather jumps left!', 'FF5555');
  ...
  export class PulseSystem {
    ...
    private reversePulseStartRadius = 26; // Starting radius for the reverse pulse
    ...
    private crackedTileChance = 0.05; // 5% chance for a block to become cracked
    ...
    private startPulse(currentTime: number) {
      ...
      // Alert all players about the new pulse
      const playerEntities = this.world.entityManager.getAllPlayerEntities();
      playerEntities.forEach(entity => {
        if (entity.player) {
          this.world.chatManager.sendPlayerMessage(entity.player, '⚠️ PULSE WAVE DETECTED! ⚠️', 'FF0000');
          // Add pulse delay info to help debugging
          this.world.chatManager.sendPlayerMessage(entity.player, `Next pulse in: ${this.pulseDelay/1000} seconds`, 'AAAAAA');
        }
      });
    ...
    private createBlockAffectedVisualEffect(position: { x: number, y: number, z: number }) {
      ...
      if (Math.random() < 0.15) { // 15% chance per block
        ...
        playerEntities.forEach(entity => {
          if (entity.player) {
            // Send a direct warning message
            this.world.chatManager.sendPlayerMessage(
              entity.player,
              `⚠️ MOVE QUICKLY! BLOCKS VANISHING! ⚠️`,
              'FF0000'
            );
          }
        });
    ...
    createCrackedTiles() {
      ...
      const maxCrackedTiles = Math.floor(platformArea * 0.1); // Maximum 10% of platform area
          ...
          for (let i = this.crackedBlocks.length - 1; i >= 0; i--) {
            ...
            if (block.position.x === playerBlockX && block.position.z === playerBlockZ) {
              ...
              if (playerEntity.player) {
                this.world.chatManager.sendPlayerMessage(
                  playerEntity.player,
                  "⚠️ Cracked block collapsed beneath you! ⚠️",
                  "FF5500"
                );
              }
            }
    ...
    private applySlowEffect(playerEntity: PlayerEntity) {
      ...
      this.world.chatManager.sendPlayerMessage(
        playerEntity.player,
        "⚠️ STICKY FLOOR! Movement slowed! ⚠️",
        "FF0000"
      );
    }
    ...
    private cleanupExpiredSlowEffects(currentTime: number) {
      ...
      for (const effect of expiredEffects) {
        ...
        if (playerEntity && playerEntity.player) {
          ...
          this.world.chatManager.sendPlayerMessage(
            playerEntity.player,
            "🏃‍♂️ Movement speed returned to normal! 🏃‍♂️",
            "FFFF00"
          );
      ...
      for (const effect of this.playerSlowEffects) {
        ...
        if (playerEntity && playerEntity.player) {
          ...
          if (Math.random() < 0.3) { // 30% chance each second
            ...
            this.world.chatManager.sendPlayerMessage(
              playerEntity.player,
              `⚠️ SLOW EFFECT: ${remainingTime}s remaining ⚠️`,
              "FFFF00"
            );
          }
        }
    ...
    private startReversePulse() {
      ...
      // Alert all players about the reverse pulse
      const playerEntities = this.world.entityManager.getAllPlayerEntities();
      playerEntities.forEach(entity => {
        if (entity.player) {
          this.world.chatManager.sendPlayerMessage(entity.player, '⚠️ REVERSE PULSE DETECTED! Coming from the edge! ⚠️', 'FF5500');
        }
      });
    }
    ...
    private startCrossPulse() {
      ...
      allPlayerEntities.forEach(entity => {
        if (entity.player) {
          this.world.chatManager.sendPlayerMessage(
            entity.player, 
            `⚠️ CROSS PULSE DETECTED! (${direction === 'NS' ? 'North-South' : 'East-West'}) ⚠️`, 
            'FF6600'
          );
        }
      });
    ...
    private startFastPulse() {
      ...
      playerEntities.forEach(entity => {
        if (entity.player) {
          this.world.chatManager.sendPlayerMessage(entity.player, '⚡ FAST PULSE DETECTED! ⚡', 'FFAA00');
        }
      });
    ...
    private startSuperPulse() {
      ...
      playerEntities.forEach(entity => {
        if (entity.player) {
          this.world.chatManager.sendPlayerMessage(entity.player, '💥 SUPER PULSE DETECTED! 💥', 'FF0000');
        }
      });
  ...
  class GameManager {
      ...
      this.buttonCheckIntervalId = setInterval(() => {
        ...
        for (const entity of playerEntities) {
          ...
          if (verticalOverlap && horizontalOverlap) {
            ...
            players.forEach(p => {
              if (p.player) {
                this.world.chatManager.sendPlayerMessage(p.player, "Game starting in 3... 2... 1...", "00FF00");
    ...
    private advanceToPhase(phase: number) {
      // The check `if (phase === this.currentPhase)` was removed as it was redundant.
      // checkPhaseProgression already ensures this is only called when phase increases.
    ...
    private triggerFallingBlocks() {
      ...
      // Determine actual number of blocks to fall (1 to maxBlockCount)
      const blockCount = Math.floor(Math.random() * maxBlockCount) + 1;
    ...
    private awardPhaseAdvancePoints() {
      ...
      playerEntities.forEach(playerEntity => {
        if (playerEntity.player) {
          ...
          this.world.chatManager.sendPlayerMessage(
            playerEntity.player,
            `+${points} points for reaching Phase ${this.currentPhase}!`,
            '00FF00'
          );
        }
      });
    ...
    private awardPulseAvoidancePoints(playerId: string, player: Player) {
      ...
      if (Math.random() < 0.3) {
        this.world.chatManager.sendPlayerMessage(
          player,
          `+${points} points for avoiding pulse wave!`,
          '00FF00'
        );
      }
    }
    ...
    public awardJumpOverPulsePoints(playerId: string) {
      ...
      if (playerEntity && playerEntity.player) {
        ...
        if (Math.random() < 0.2) {
          this.world.chatManager.sendPlayerMessage(
            playerEntity.player,
            `+${points} points for jumping over pulse!`,
            '00FF00'
          );
        }
      }
    ...
    private broadcastLeaderboard(currentTime: number) {
      ...
      // Only send chat message if leaderboard has entries
      if (this.leaderboard.length > 0) {
        this.world.chatManager.sendBroadcastMessage('━━━━━ LEADERBOARD ━━━━━', '00FFFF');
        this.leaderboard.slice(0, 5).forEach((entry, index) => {
          this.world.chatManager.sendBroadcastMessage(
            `#${index + 1}: ${entry.name} - ${entry.score} points (${entry.survivalTime.toFixed(1)}s)`, 
            index === 0 ? 'FFFF00' : 'FFFFFF'
          );
        });
        this.world.chatManager.sendBroadcastMessage('━━━━━━━━━━━━━━━━━━━━━', '00FFFF');
      }
    }
    ...
    public restartPlayer(playerId: string): void { 
      ...
      if (this.worldManager) {
        ...
        console.log(`[RestartPlayer] Requested feather jump reset for player ${playerId} via WorldManager`);
      } else {
        console.error(`[RestartPlayer] WorldManager reference not found in GameManager for player ${playerId}`);
        // Fallback: Try sending UI message directly
        if (player && player.ui) { // Use the 'player' variable available in this scope
          player.ui.sendData({
            type: 'updateFeatherJumps',
            jumpsLeft: 2
          });
          console.warn(`[RestartPlayer] Fallback: Sent UI update directly for player ${playerId}`);
        }
      }
      // <<< END RE-ADD >>>
      ...
      if (player.world) {
        player.world.chatManager.sendPlayerMessage(player, 'Welcome back to Pulse Floor+!', '00FF00');
      }
    }
  ...
  /**
    ...
    public async playerJoined(playerId: string, player: Player): Promise<void> {
      ...
      // Send welcome message to player if world is defined
      if (player.world) {
        player.world.chatManager.sendPlayerMessage(player, 'Welcome to Pulse Floor+!', '00FF00');
      }
    ...
    private awardBonusTilePoints(playerId: string, player: Player): void {
      ...
      this.world.chatManager.sendPlayerMessage(player, `+${this.bonusTilePoints} bonus points!`, '00FF00');
    }
    ...
    private triggerPlayerDeath(playerId: string, survivalTime: number): void {
       ...
       this.world.chatManager.sendPlayerMessage(player, 'Eliminated! You were out of bounds for too long.', 'FF0000');
       this.world.chatManager.sendPlayerMessage(player, `Survived for: ${survivalTime.toFixed(1)} seconds`, 'FFFF00');
       this.world.chatManager.sendPlayerMessage(player, `Final Score: ${finalScore}`, '00FF00');
    ...
    private setupPlayerEventHandlers(playerWorld: World, player: Player, gameManager: GameManager) {
      ...
      playerWorld.on(PlayerEvent.JOINED_WORLD, ({ player }) => {
        ...
        player.ui.on(PlayerUIEvent.DATA, ({ data }: { data: any }) => {
          if (data && data.type === 'restartGame') {
          ...
          } else if (data && data.type === 'playerName') {
            ...
            if (data.name && data.name.trim() !== '') {
              ...
              playerWorld.chatManager.sendPlayerMessage(player, `Welcome, ${data.name}! Let's play Pulse Floor+!`, '00FF00');
            }
            ...
            if (data.playerName) {
              ...
              playerWorld.chatManager.sendPlayerMessage(
                player, 
                `Welcome, ${data.playerName}! Get ready to dodge the pulses!`, 
                '00FF00'
              );
            }
        ...
        // Send welcome messages with clear instructions about the pulse mechanics
        playerWorld.chatManager.sendPlayerMessage(player, '━━━━━ WELCOME TO PULSE FLOOR+ ━━━━━', '00FF00');
        playerWorld.chatManager.sendPlayerMessage(player, 'Watch for the pulse wave that spreads from the central altar!', 'FFFF00');
        playerWorld.chatManager.sendPlayerMessage(player, 'If you\'re standing on a block when the pulse passes over it...', 'FFFF00');
        playerWorld.chatManager.sendPlayerMessage(player, 'The block will glow red and disappear after 1 second!', 'FF5500');
        playerWorld.chatManager.sendPlayerMessage(player, 'You must jump to a different block before it vanishes!', 'FFFF00');
        playerWorld.chatManager.sendPlayerMessage(player, 'Use WASD to move and SPACE to jump.', 'FFFFFF');
        playerWorld.chatManager.sendPlayerMessage(player, 'Earn points by surviving, avoiding pulses, and jumping over them!', '00FFFF');
        // Add message about the feather
        playerWorld.chatManager.sendPlayerMessage(player, 'Your feather allows you to double jump! Press right-click while in the air to use it.', '00FFFF');
        playerWorld.chatManager.sendPlayerMessage(player, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '00FF00');
  ...
  startServer(world => {
    ...
    world.on(PlayerEvent.JOINED_WORLD, ({ player }) => {
      ...
      world.chatManager.sendPlayerMessage(player, 'Welcome to Pulse Floor+! Creating your personal game world...', '00FF00');
